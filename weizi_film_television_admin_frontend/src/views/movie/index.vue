<template>
    ...
    <el-row>
      ...
      <el-col :span="6">
        <el-form-item label="功能：">
          ...
          <el-button type="warning" @click="editMovieGroupDialogShow = true">批量编辑</el-button>
        </el-form-item>
      </el-col>
    </el-row>
  </el-form>
  ...
  >
      ...
      <template #default="scope">
        <div style="text-align:center;">
          <el-image
              ...
              :src="scope.row.mainPoster || defaultPoster"
              fit="contain"
              preview-teleported="true"
              :preview-src-list="[scope.row.mainPoster || defaultPoster]"
        ...
        <div style="text-align:center;">
          ...
          <el-tag v-if="scope.row.channelId" round type="info">
            {{ channelGroup[scope.row.channelId] }}
          </el-tag>
        </div>
      </template>
    </el-table-column>
    ...
    <el-table-column fixed="right" align="center" label="操作">
        <template #default="scope">
          ...
          <el-button type="success" size="small" @click="handleMovieEdit(scope.row.movieId)">编辑</el-button>
          <el-button v-if="scope.row.isPlayer" type="danger" size="small" @click="handleMovieRemove(scope.row.movieMd5, scope.row.movieName)">删除</el-button>
        </template>
    </el-table-column>
  </el-table>
  <!-- 分页 -->
  ...
  <el-dialog v-model="editMovieGroupDialogShow" title="批量编辑" width="400" :show-close="false" center>
    <el-form :model="editMovieGroupForm" :rules="editMovieGroupFormRules" label-width="120px" ref="editMovieGroupFormRef" style="padding: 10px">
      <el-form-item label="影片状态" prop="status">
        <el-switch
            v-model="editMovieGroupForm.status"
            inline-prompt
            active-text="上架"
            inactive-text="下架"
            :active-value="false"
      ...
      <el-form-item label="影片频道" prop="channelId">
        <el-select
            v-model="editMovieGroupForm.channelId"
            placeholder="请选择频道"
        >
          <el-option
              v-for="(value, key) in channelGroup"
              :key="key"
              :label="value"
              :value="parseInt(key)"
          />
    ...
    <template #footer>
          <span class="dialog-footer">
              <el-button @click="handleEditMovieGroupClose">取消</el-button>
              <el-button type="primary" @click="handleEditMovieGroupFormSubmit">提交</el-button>
          </span>
    </template>
  </el-dialog>
  <!-- 编辑的抽屉 -->
  <el-drawer v-model="movieFormShow" title="编辑影片信息" width="50%" :before-close="handleMovieFormClose">
    <!-- 表单 -->
    <el-form :model="form" :rules="rules" label-width="120px" ref="formRef" style="padding-right: 10px">
      <el-row>
        <el-col :span="24">
          <!-- 影片海报编辑时显示 -->
          <el-form-item title="主海报" prop="mainPoster">
            <el-upload
                action="#"
                ref="uploadRef"
            ...
            >
              <img @error="handleFormPosterImageError" :src="form.mainPoster || defaultPoster" alt="" style="max-width: 200px; max-height: 200px;">
            </el-upload>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
          <!-- 影片海报编辑时显示 -->
          <el-form-item title="海报（视频截图）" prop="poster">
            <el-upload
                action="#"
                ref="uploadRef"
                :show-file-list="false"
                ...
                :before-upload="beforePosterUpload"
            >
              <img @error="handleFormPosterImageError" :src="form.poster || defaultPoster" alt="" style="max-width: 200px; max-height: 200px;">
            </el-upload>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <template #footer>
          <span class="dialog-footer">
              <el-button @click="handleMovieFormClose">取消</el-button>
...
onMounted(() => {
  ...
  getChannelMap().then(res => {
    if (res.data.code === 200) channelGroup.value = res.data.data;
  });
  getGenreTagMap().then(res => {
    if (res.data.code === 200) genreGroup.value = res.data.data;
  }).finally(
      () => {
...
// 响应式变量
let tableLoading = ref(false);          // 表格加载状态
const playerDialogVisible = ref(false); // 播放器弹窗显示状态
let movieFormShow = ref(false);         // 影片表单显示状态
let editMovieGroupDialogShow = ref(false);   // 编辑组弹窗显示状态
let movieList = ref([]);                // 影片列表
let selectMovieIds = ref([]);           // 选中的影片ID列表
let channelGroup = ref({});             // 频道组
let genreGroup = ref({});               // 类型组
let total = ref(0);                     // 总条数
const formRef = ref(null);              // 表单
const editMovieGroupFormRef = ref(null);     // 编辑组表单
let queryForm = ref({
  movieName: undefined,
  pageNum: 1,
  pageSize: 10
...
// 初始化编辑组表单值
const initialEditMovieGroupFormValue = {
  status: false,
  channelId: undefined,
  movieIdList: undefined
};

// 编辑数据时的表单数据
let form = ref({...initialFormValue});
let editMovieGroupForm = ref({...initialEditMovieGroupFormValue});
...
// 表单验证规则
let editMovieGroupFormRules = ref({
  status: [
    { type: 'boolean', message: '请选择状态', trigger: 'change' }
  ],
  channelId: [
...
function handleSelectionChange(selection) {
  selectMovieIds.value = selection.map(item => item.movieId);
}
...
function handleEditMovieGroupFormSubmit() {
  editMovieGroupFormRef.value.validate((valid) => {
    if (valid) {
      if (selectMovieIds.value.length < 1) {
        ElMessage.error('请选择至少一部影片！');
        return;
      }
      editMovieGroupForm.value.movieIdList = selectMovieIds.value;
      // 提交表单
      updateMovieGroup(editMovieGroupForm.value).then(res => {
        if (res.data.code === 200) {
          // 弹窗提示新增成功
          ElMessage.success("编辑组成功！");
          // 刷新列表
          handleSearchMovieList();
          // 关闭窗口
          handleEditMovieGroupClose();
        }
      })
      }
  })
...
// 关闭编辑组弹窗
function handleEditMovieGroupClose() {
  editMovieGroupDialogShow.value = false;
  editMovieGroupForm.value = {...initialEditMovieGroupFormValue};
  // 清空表单验证状态和错误信息
  editMovieGroupFormRef.value?.resetFields();
}

// 关闭弹窗
function handleMovieFormClose() {
  movieFormShow.value = false;
...
function handleMovieRemove(movieMd5, movieName) {
...
async function uploadMainImage(file) {
  ...
  try {
    const response = await uploadMainPoster(formData);
    if (response.status === 200) {
      // 使用FileReader读取文件并显示
      const reader = new FileReader();
      reader.onload = (e) => {
        // 将Base64编码的URL赋值给form.posterUrl
        form.value.mainPoster = e.target.result;
      };
