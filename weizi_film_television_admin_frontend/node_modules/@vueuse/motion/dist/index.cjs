...
class MotionValue {
  ...
  constructor(init) {
     ...
     */
    __publicField(this, "scheduleVelocityCheck", () => sync__default.postRender(this.velocityCheck));
    /**
   ...
   */
  getVelocity() {
    return this.canTrackVelocity ? popmotion.velocityPerSecond(Number.parseFloat(this.current) - Number.parseFloat(this.prev), this.timeDelta) : 0;
  }
  /**
...
const supportsPointerEvents = () => isBrowser && (window.onpointerdown === null || undefined.TEST);
const supportsTouchEvents = () => isBrowser && (window.ontouchstart === null || undefined.TEST);
const supportsMouseEvents = () => isBrowser && (window.onmousedown === null || undefined.TEST);
...
function registerEventListeners({ target, state, variants, apply }) {
  ...
  vue.watch([hovered, tapped, focused], () => {
...
function registerVisibilityHooks({ target, variants, variant }) {
  ...
  if (_variants && (_variants.visible || _variants.visibleOnce)) {
    core.useIntersectionObserver(target, ([{ isIntersecting }]) => {
  ...
  vue.watch(
    ...
    (newVal) => {
      ...
      if (enableHardwareAcceleration && (newVal.x || newVal.y || newVal.z)) {
        const str = [newVal.x || 0, newVal.y || 0, newVal.z || 0].map((val) => getValueAsType(val, styleValueTypes.px)).join(",");
...
function directive(variants, isPreset = false) {
  const register = (el, binding, node) => {
    ...
    const variantsRef = vue.ref(variantsObject);
...
function setupMotionComponent(props) {
  ...
  const preset = vue.computed(() => props.preset ? structuredClone(presets[props.preset]) : {});
  ...
  const motionConfig = vue.computed(() => {
    const config = defu__default({}, propsConfig.value, preset.value, props.variants || {});
...
const MotionComponent = vue.defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      ...
      const node = vue.h(props.is, void 0, slots);
...
const MotionGroupComponent = vue.defineComponent({
  props: {
    ...MotionComponentProps,
    is: {
      type: [String, Object],
  ...
  setup(props) {
    ...
    return () => {
      ...
      if (props.is) {
        return vue.h(props.is, void 0, nodes);
      }
      return nodes;
    };
  }
...
exports.MotionGroupComponent = MotionGroupComponent;
