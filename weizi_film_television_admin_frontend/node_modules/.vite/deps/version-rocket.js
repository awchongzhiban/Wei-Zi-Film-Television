...
// node_modules/.pnpm/version-rocket@1.7.1/node_modules/version-rocket/components/versionTipDialog.js
import versionBg from "D:/allItem/rookieJava/weizi_film_television/weizi_film_television_admin_frontend/node_modules/.pnpm/version-rocket@1.7.1/node_modules/version-rocket/assets/version-bg.png";
import "D:/allItem/rookieJava/weizi_film_television/weizi_film_television_admin_frontend/node_modules/.pnpm/version-rocket@1.7.1/node_modules/version-rocket/components/version-tip-dialog.css";

// node_modules/.pnpm/version-rocket@1.7.1/node_modules/version-rocket/components/versionTipTheme.js
var setVersionTipTheme = (primaryColor = "#FA8D88", rocketColor = "#FE7D66") => {
  ...
  const template = `
   <div id="version-rocket">
        <div class="version-area">
            ${params.primaryColor || params.rocketColor ? `<div class="version-img">${setVersionTipTheme(params.primaryColor, params.rocketColor)}</div>` : `<img class="version-img" src="${params.imageUrl || versionBg}" alt="version" />`} 
            <div class="version-content">
                ...
                <div style="${params.primaryColor ? `background-color: ${params.primaryColor};` : ""} ${params.buttonStyle || ""}"  class="refresh-button">
                  ${params.buttonText || defaultParams.buttonText}
                </div>
                ${params.cancelButtonText ? `<div class="cancel-button">
  ...
  cancelBtnNode.onclick = () => {
    ...
    switch (cancelMode) {
      case "ignore-current-version":
        ...
        break;
      case "ignore-today":
        localStorage.setItem("version-rocket:cancelled", (/* @__PURE__ */ new Date()).toLocaleDateString());
        break;
...
// node_modules/.pnpm/version-rocket@1.7.1/node_modules/version-rocket/utils/index.js
var createWorker = (func, deps) => {
  const depsFuncStr = `${(deps === null || deps === void 0 ? void 0 : deps.map((_) => _.toString()).join(";\n\n")) || ""}`;
  const blob = new Blob([
    `
    ${depsFuncStr};
    (${func.toString()})();
...
var createWorkerFunc = () => {
  let oldVersion = "";
  const flagSet = /* @__PURE__ */ new Set();
  let intervalTime = 5e3;
  let immediate = false;
  let originFileUrl = "";
  ...
  temp.onmessage = (event) => {
    ...
    checkOriginSpecifiedFilesUrl = event.data["check-origin-specified-files-url"];
    checkOriginSpecifiedFilesUrlMode = event.data["check-origin-specified-files-url-mode"];
    ...
    const doFetch = () => {
      if (checkVersionType === "check-version") {
        fetch(`${originFileUrl}?${+/* @__PURE__ */ new Date()}`).then((res) => {
          return res.json();
      ...
      if (checkVersionType === "check-specified-files") {
        ...
        checkOriginSpecifiedFilesUrl.forEach((url) => {
          fetch(url, {
            ...
            if (checkOriginSpecifiedFilesUrlMode === "one" && flagSet.size > checkOriginSpecifiedFilesUrl.length) {
              temp.postMessage({
                refreshPageVisible: true,
                refreshPageVersion: "",
                external: ""
            ...
            if (checkOriginSpecifiedFilesUrlMode === "all" && flagSet.size === checkOriginSpecifiedFilesUrl.length * 2) {
              temp.postMessage({
                refreshPageVisible: true,
                refreshPageVersion: "",
                external: ""
...
var cancelUpdateFunc = (cancelMode, newVersion, cancelUpdateAndStopWorker, worker2) => {
  const cancelModeType = cancelMode || (newVersion ? "ignore-current-version" : "ignore-current-window");
  const cancelModeTypeValue = localStorage.getItem("version-rocket:cancelled") || "";
  const todayDate = (/* @__PURE__ */ new Date()).toLocaleDateString();
  ...
  switch (cancelModeType) {
    case "ignore-current-version":
      if (newVersion && cancelModeTypeValue === newVersion) {
        isStopWorker && (worker2 === null || worker2 === void 0 ? void 0 : worker2.terminate());
        return true;
      }
    case "ignore-today":
...
function checkVersionTypeFunc(oldVersion, originFileUrl, checkOriginSpecifiedFilesUrl) {
  const checkVersionType = oldVersion && originFileUrl ? "check-version" : (checkOriginSpecifiedFilesUrl === null || checkOriginSpecifiedFilesUrl === void 0 ? void 0 : checkOriginSpecifiedFilesUrl.length) ? "check-specified-files" : "";
  if (!checkVersionType)
...
// node_modules/.pnpm/version-rocket@1.7.1/node_modules/version-rocket/index.js
var pollingCompareVersion = (localPackageVersion, originVersionFileUrl, pollingTime, onVersionUpdate) => {
  const worker2 = createWorker(createWorkerFunc);
  worker2.postMessage({
...
var worker = void 0;
var checkVersion = (config, options) => {
  ...
  worker.postMessage({
    ...
    "polling-time": config.pollingTime || 5e3,
...
var unCheckVersion = ({ closeDialog = false, closeWorker = true }) => {
  if (closeWorker) {
...
//# sourceMappingURL=version-rocket.js.map
