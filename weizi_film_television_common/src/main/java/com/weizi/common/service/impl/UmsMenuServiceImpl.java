...
@Service
public class UmsMenuServiceImpl extends ServiceImpl<UmsMenuMapper, UmsMenuEntity> implements IUmsMenuService {
    ...
    public List<RouterVO> searchSelfMenu() {
        ...
        List<UmsMenuEntity> menuList = baseMapper.selectAdminByRoleIds(roleIds);
        // 通过递归设置菜单的树形结构
    ...
    public WeiZiPageResult<UmsMenuEntity> selectList(UmsMenuParamDto menuParamDto) {
        ...
        // 这里递归把子数据放入到对应的父级数据中
        List<UmsMenuEntity> menusList = _buildMenuChildren(record);
    ...
    @Override
    public UmsMenuEntity searchMenuById(Long menuId) {
        return baseMapper.selectByMenuId(menuId);
    }

    @Override
    public WeiZiResult saveMenu(UmsMenuEntity umsMenuEntity) {
        // 判断是否存在
        if (ObjectUtil.isNull(umsMenuEntity.getParentId())) umsMenuEntity.setParentId(0L);
        int count = baseMapper.checkCount(umsMenuEntity.getParentId(), umsMenuEntity.getMenuName(), umsMenuEntity.getPath(), umsMenuEntity.getPerms(), null);
        if (count > 0) {
            return WeiZiResult.error(HttpStatus.ERROR_MESSAGE, "该菜单已存在，或路径已存在！");
        }
        baseMapper.insert(umsMenuEntity);
        return WeiZiResult.success();
    }

    @Override
    public WeiZiResult updateMenu(UmsMenuEntity umsMenuEntity) {
        // 判断是否存在
        UmsMenuEntity existingMenu = baseMapper.selectByMenuId(umsMenuEntity.getMenuId());
        if (existingMenu == null) {
            return WeiZiResult.error("要更新的菜单不存在！");
        }
        if (ObjectUtil.isNull(umsMenuEntity.getParentId())) umsMenuEntity.setParentId(0L);
        int count = baseMapper.checkCount(umsMenuEntity.getParentId(), umsMenuEntity.getMenuName(), umsMenuEntity.getPath(), umsMenuEntity.getPerms(), existingMenu.getMenuId());
        if (count > 0) {
            return WeiZiResult.error("该菜单已存在，或路径已存在！");
        }
        baseMapper.updateById(umsMenuEntity);
        return WeiZiResult.success();
    }
    ...
    public List<RouterVO> selectMenuList() {
        List<UmsMenuEntity> menuList = baseMapper.selectList(null);
        // 渲染children
        return _getRouter(menuList);
    }
     ...
     * 获取路由
     */
    private List<RouterVO> _getRouter(List<UmsMenuEntity> menuList) {
        List<RouterVO> routerVOS = new ArrayList<>();
        // 首先获取所有的1级路由
        List<UmsMenuEntity> parentMenu = menuList.stream().filter(item -> item.getParentId() == 0).toList();
        // 转换对象类型
        for (UmsMenuEntity menu : parentMenu) {
     ...
     */
    private List<RouterVO> _buildTree(List<UmsMenuEntity> allMenu, Long parentId) {
        List<RouterVO> childrenList = new ArrayList<>();
        // 遍历所有的数据
        for (UmsMenuEntity menu : allMenu) {
     ...
     * 构建子菜单
     */
    private List<UmsMenuEntity> _buildMenuChildren(List<UmsMenuEntity> menuList) {
        // 首先获取所有的1级路由
        List<UmsMenuEntity> parentMenu = menuList.stream().filter(item -> item.getParentId() == 0).collect(Collectors.toList());
        // 循环1级路由，获取所有的子菜单
        for (UmsMenuEntity menuEntity : parentMenu) {
            // 获取所有的子节点
            List<UmsMenuEntity> childrenList = _buildMenuTree(menuList, menuEntity.getMenuId());
     ...
     * 获取所有子节点，递归获取【如果是2级不需要递归了】
     */
    private List<UmsMenuEntity> _buildMenuTree(List<UmsMenuEntity> allMenu, Long parentId) {
        List<UmsMenuEntity> childrenList = new ArrayList<>();
        // 遍历所有的数据
        for (UmsMenuEntity menu : allMenu) {
            // 判断menu的parentId是否与传进来的parentId相同
            if (menu.getParentId().equals(parentId)) {
        ...
        // 递归childrenList可能还有子节点
        for (UmsMenuEntity childrenItem : childrenList) {
    ...
    private WeiZiPageResult<UmsMenuEntity> _selectPage(UmsMenuParamDto menuParamDto) {
        ...
        // 执行分页查询
        List<UmsMenuEntity> records = baseMapper.selectMenuPage(menuParamDto.getMenuName(), menuParamDto.getPerms(), pageNum, menuParamDto.getPageSize());

        // 获取到最外层父级Id数组
        List<Long> menuIdList = records.stream().map(UmsMenuEntity::getMenuId).toList();
        if (ObjectUtil.isNotEmpty(menuIdList)) {
            // 获取所有子菜单数组
            List<UmsMenuEntity> childrenMenus = baseMapper.getChildrenMenu(menuIdList);
            // 判空后加入到records内
